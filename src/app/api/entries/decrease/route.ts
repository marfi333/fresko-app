...
export const POST = async (request: Request) => {
  ...
  if (!body.productId || !body.amount || body.amount <= 0) {
    return NextResponse.json({ error: "productId and amount (> 0) are required" }, { status: 400 });
  }

  const productEntries = await ctx.db
    .select()
    .from(entries)
    .where(and(eq(entries.productId, body.productId), eq(entries.householdId, ctx.householdId)))
    .orderBy(asc(entries.expiryDate));

  if (productEntries.length === 0) {
    return NextResponse.json({ error: "No entries found for this product" }, { status: 404 });
  }

  const totalAvailable = productEntries.reduce((sum, e) => sum + e.quantity, 0);
  if (body.amount > totalAvailable) {
    return NextResponse.json({ error: "Amount exceeds available quantity" }, { status: 400 });
  }
